class Graph {
	...
	constructor(gl, text) {
		...
		this.lineShader = 'attribute vec2 aPos; attribute vec4 aColor; uniform mat3 uMatrix; varying lowp vec4 vColor; void main(void) { float x = aPos.x; float y = aPos.y; x = X; y = Y; float z = 0.0; vec2 position = (uMatrix * vec3(x, y, 1.0)).xy; vec4 color = aColor; if (abs(position.x) > 1.1 || abs(position.y) > 1.1) { color = vec4(0.0, 0.0, 0.0, 0.0); z = 1.0; } gl_Position = vec4(position.xy, z, 1.0); vColor = color; }';
	...
	render() {
		...
		canvas.renderLineX(-centerX - oneScaledX, -centerX + oneScaledX, 1, new Color(0, 0, 0, 1));
		canvas.flush('LINE', true, vertex, fragment, this.time);

		canvas.renderLineY(-centerY - oneScaledY, -centerY + oneScaledY, 1, new Color(0, 0, 0, 1));
		canvas.flush('LINE', true, vertex, fragment, this.time);
