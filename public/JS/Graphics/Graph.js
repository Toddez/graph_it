...
class Graph {
	...
	constructor(gl, text) {
		...
		this.pointShader = 'attribute vec2 aPos; attribute vec4 aColor; uniform mat3 uMatrix; uniform vec2 uRes; varying lowp vec4 vColor; varying lowp vec2 vRes; varying lowp vec2 vPos; void main(void) { vec2 pos = vec2POS; vec2 position = (uMatrix * vec3(pos.x, pos.y, 1.0)).xy; gl_Position = vec4(position.xy, 0.0, 1.0); vPos = position; vColor = aColor; gl_PointSize = 7.0; vRes = uRes; }';
